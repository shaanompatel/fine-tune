...
def parse_args():
    ...
    if args.dataset_name is None and args.train_file is None:
        ...
        if args.train_file is not None:
            ...
            assert extension in ["json", "jsonl", "txt"], "`train_file` should be a json/jsonl file."
    return args


def encode_unsupervised(example, tokenizer, max_seq_length, add_bos=False):
    '''
    Here we assume each example contains a 'text' field with raw, unlabeled text.
    We treat the entire example as a sequence and tokenize it.
    '''
    example_text = example['text']
    example_text = example_text + tokenizer.eos_token  # Add end of sequence token
    
    if add_bos:
        example_text = tokenizer.bos_token + example_text  # Add beginning of sequence token if required
    
    # Tokenize the text
    tokenized_example = tokenizer(example_text, return_tensors='pt', max_length=max_seq_length, truncation=True)
    input_ids = tokenized_example.input_ids
    attention_mask = torch.ones_like(input_ids)  # Attention mask (1 for tokens, 0 for padding)

    # For unsupervised learning, the labels can be the same as input_ids (next-token prediction)
...
def load_chunks(file_path, lines_per_chunk=10):
    """Loads a file and groups it into chunks of lines."""
    chunks = []
    current_chunk = []
...
def main():
    ...
    if args.dataset_name is not None:
        logger.info(f"Loading dataset from hub")
        ...
        logger.info(f"Opening local dataset")
        data_files = {}
        dataset_args = {}
        if args.train_file is not None:
            data_files["train"] = args.train_file
        try:
            raw_datasets = load_chunks(args.train_file, lines_per_chunk=10)
            ...
            # Optionally, filter out overly long text sequences if needed
            max_length = 400  # Customize this limit based on your requirements
            raw_datasets["train"] = raw_datasets["train"].filter(lambda x: len(x["text"].split()) < max_length)

            # Sample and log some training data for verification
            logger.info(f"After filtering, there are {len(raw_datasets['train'])} examples in the training set.")
            logger.info(f"Sample of the training set: {raw_datasets['train'][0]}")

        except Exception as e:
            logger.error(f"Error loading data from {args.train_file}: {e}")
    ...
    if "prompt" in raw_datasets["train"].column_names and "completion" in raw_datasets["train"].column_names:
        ...
        raise ValueError("You need to have either 'prompt'&'completion' or 'messages' or 'text' in your column names.")
    ...
    if args.resume_from_checkpoint:
        ...
        if "epoch" in training_difference:
            ...
            # Multiply by gradient_accumulation_steps to reflect real steps
            resume_step = int(training_difference.replace("step_", "")) * args.gradient_accumulation_steps
            starting_epoch = resume_step // len(train_dataloader)
            completed_steps = resume_step // args.gradient_accumulation_steps
            resume_step -= starting_epoch * len(train_dataloader)

    # Update the progress_bar if loaded from a checkpoint
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(active_dataloader):
            with accelerator.accumulate(model):
                # Use the input as both the source and target (for unsupervised tasks)
                inputs = batch["input_ids"]
                attention_mask = batch["attention_mask"]

                # Example of Masked Language Modeling loss
                # Assume inputs are the source and target for the MLM task
                outputs = model(input_ids=inputs, attention_mask=attention_mask, labels=inputs)  # Labels can be the same as inputs
                loss = outputs.loss
